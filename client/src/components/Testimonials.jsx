...
const testtis = [
    {user: "Samay", image:'https://upload.wikimedia.org/wikipedia/commons/d/d0/Samay_raina.jpg', feedback : "This platform makes lesson planning faster and smarter. It helps me create engaging classes with AI-powered insights. "},
  ...
  return (
    <motion.div variants={testiAnimation} initial='initial' whileInView='inView'>
        <ul className='grid sm:grid-cols-5 sm:gap-10 gap-5 mt-10 px-5 '>
            {
                testtis.map(testis => (
                    <li key={testis.user} className='bg-gradient-to-r from-blue-300 to-blue-200 to-[#ddeefa] p-8 rounded-xl'>
                        <div className='text-[14px]'>"{testis.feedback}"</div>
                        <div className='flex justify-center items-center mt-5 gap-3'>
                            <img src={testis.image} width={30} height={40} alt="" className='rounded-xl' />
                            <p className='text-xs'>{testis.user}</p>
                        </div>
                    </li>
                ))
            }
